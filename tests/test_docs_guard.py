...
The template's own gate proves the guard passes on the shipped documentation
(via test_generated_docs_guard_runs_and_passes); these tests prove each rule
fires on planted violations and stays silent on legitimate prose. They go
through docs_guard.check_documentation so the composition under test is the
shipped one: one markdown walk, all rule families.
"""
...
def make_repo(tmp_path: Path) -> Path:
    ...
    template_adr = (TEMPLATE / "docs" / "adr" / "0000-template.md").read_text(encoding="utf-8")
    (adr_dir / "0000-template.md").write_text(template_adr, encoding="utf-8")
