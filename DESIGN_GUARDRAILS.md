...
| Domain invariants | immutable value objects and pure transitions with named pre/postconditions | frozen/slotted core state; `proof.toml` closes the public proof surface; independent predicates feed `icontract`, Hypothesis proofs, falsifying canaries, and mandatory CrossHair targets for contracted functions and methods; ARCH027 rejects exact same-named Enum copies and ARCH029 rejects used domain aliases to bare primitives |
