...
| Keep routine quality checks consistent | `just check`, prek hooks, and the generated quality gate run formatting, linting, type checks, ownership, architecture, documentation, imports, tests, and coverage; `prek.toml` pins Python hooks to 3.14 so their parser matches the repository language contract. |
| Certify recursive self-generation and teach only the exercised workflow | `tests/recursive/test_recursive_generation.py` drives one real N0 → N1 → N2 walk through `repoctl` and checks the downstream agent workflow against its ordered invocation log; the complete pre-push pack suite therefore has a ten-minute warm-cache budget while pre-commit keeps the sub-minute lane. |
