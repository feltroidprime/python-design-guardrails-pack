...
| Local enforcement from repository creation onward | bootstrap before the first commit; use Git's shared hooks directory for linked worktrees | `python-repo init` runs `just bootstrap` after `git init` and before the baseline commit; pack validation proves the installed prek commit and pre-push shims resolve identically from a newly added linked worktree |
