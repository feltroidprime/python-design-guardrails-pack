...
| Keep the capability layout checkable by a maintained tool | The three hand-written rules `CAP001` to `CAP003` are gone. Each one is now a contract, so a defect report comes from `import-linter` rather than from pack code. |
...
| Keep comments free of scheduled manual upkeep | Ruff's `TD` and `FIX` families. They replace the hand-written `ARCH031` rule and its edit-time hook, so a maintained tool carries the rule instead of pack code. `ARCH024` and `ARCH025` also go: the router derives the command surface, so no catalog registration remains to guard. |
...
No coverage assertion exists in this tree or in a projected project. `--cov`,
`pytest-cov`, and `[tool.coverage` are banned in both. Conflict C10 of #85
settles it, and this section is the rationale that #85 requires.
