...
# release: bytecode caches, tool caches, the forced-update backups, and the
# staged projection blob. The manifest records none of them, so a drift scan
# reads none of them either. Both zones use one list, so a cache directory can
# never enter a manifest and refuse every later update.
