...
# Install this repo's prek hooks (pre-commit + pre-push) if they aren't
# already wired up. Safe to re-run; skips when both hooks are already
# prek-managed.
hooks:
    #!/usr/bin/env bash
    set -euo pipefail
    if [ -x .git/hooks/pre-commit ] && [ -x .git/hooks/pre-push ] \
        && grep -qs 'generated by prek' .git/hooks/pre-commit \
        && grep -qs 'generated by prek' .git/hooks/pre-push; then
        echo "prek hooks already installed"
    else
        uv run --no-project --python 3.14 --with prek>=0.4.9 prek install -f
    fi
